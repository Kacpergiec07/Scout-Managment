...
// Truncate at the start of PlayerDot
const start = content.indexOf('function PlayerDot');
if (start !== -1) {
    content = content.substring(0, start);
}

const playerDot = `function PlayerDot({ player, subs }: { player: any, subs: any[] }) {
...
fs.writeFileSync(path, content + playerDot);
