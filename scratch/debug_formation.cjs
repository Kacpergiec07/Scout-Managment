const { StatoriumClient } = require('../lib/statorium/client.ts');
        ...
        try {
          ...
          if (ourLineup.length > 0) {
            ...
            ourLineup.slice(0, 3).forEach((player, idx) => {
              console.log(`     ${idx + 1}. ${player.playerName || player.name} - Pos: ${player.additionalInfo?.Position || player.position}`);
            });
          } else {
