...
// Update PlayerDot calls in TeamLineupView
content = content.replace(/player={p} \/>/g, 'player={p} subs={subs} />');

// Update PlayerDot function definition
const oldPlayerDot = `function PlayerDot({ player }: { player: any }) {
   return (
...
const newPlayerDot = `function PlayerDot({ player, subs }: { player: any, subs: any[] }) {
...
content = content.replace(oldPlayerDot, newPlayerDot);
