    ...
    <aside className="hidden w-64 flex-col border-r border-zinc-800 bg-zinc-900 md:flex">
      ...
      <div className="border-t border-zinc-800 p-4">
        ...
        <form>
          <button
            formAction={signOut}
            className="flex w-full items-center gap-3 rounded-md px-3 py-2 text-sm font-medium text-zinc-400 hover:bg-zinc-800 hover:text-zinc-50 transition-colors"
          >
...
function SidebarItem({ href, icon: Icon, label }: { href: string; icon: any; label: string }) {
  return (
    <Link
      href={href}
      className="flex items-center gap-3 rounded-md px-3 py-2 text-sm font-medium text-zinc-400 hover:bg-zinc-800 hover:text-zinc-50 transition-colors"
    >
